...
def main():
    ...
    for k in range(int(args.K)):
        if args.nodes_per_color == "-1":
            S, f, fc, kb = tUFLP.generate_test_instance(int(args.n), float(args.prob))
        else:
            S, f, fc, kb = tUFLP.generate_test_instance_alt(int(args.n), float(args.prob),
                                                           nodes_per_color=int(args.nodes_per_color))

        cover_nat, _ = tUFLP.build_cover_DD(S, f)
        pref_order = [int(x[1:]) for x in cover_nat.vars]
        color_nat, _ = tUFLP.build_type_DD(f, fc, kb, pref_order)

        color_nat.make_reduced()
        cover_nat.make_reduced()

        cover_rnd, _ = tUFLP.build_randomized_cover_DD(S, f)
        color_rnd, _ = tUFLP.build_randomized_type_DD(f, fc, kb)
