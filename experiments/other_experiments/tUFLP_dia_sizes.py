"""Benchmarks DD sizes for different solution methods (typed UFLP)

An experiment concerning the Uncapacitated Facility Location with types (typed UFLP):

    - compares diagram sizes for type, covering, and intersection DDs
...
def main():
    ...
    for k in range(int(args.K)):
        ...
        if args.inst_type == "ER":
            ...
            S, f, fc, kb = tUFLP.generate_string_instance(int(args.n))
        else:
            print(f"FATAL ERROR: {args.inst_type} -- wrong instance type. Expected `ER` or `STRING`.")
            exit(-1)

        cover, _ = tUFLP.build_cover_DD(S, f)

        pref_order = [int(x[1:]) for x in cover.vars]
        color, _ = tUFLP.build_type_DD(f, fc, kb, pref_order)
