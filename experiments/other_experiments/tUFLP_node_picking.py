...
in (typed) UFL (random, min, and max degrees).
...
def get_sizes(n=10, K=1):
    ...
    for k in range(K):
        S, f, fc, kb = tUFLP.generate_test_instance(n)
        cov_min, _ = tUFLP.build_cover_DD(S, f)
        cm = deepcopy(cov_min)
        cov_min.make_reduced()
        assert cm.is_equivalent(cov_min)[0], f"S={S}; f={f} # cov_min problem"

        cov_max, _ = tUFLP.build_cover_DD(S, f, next_node_type="max")
        cov_max.make_reduced()
        assert cov_max.is_equivalent(cov_min)[0], f"S={S}; f={f} # cov_max problem"

        cov_rnd, _ = tUFLP.build_cover_DD(S, f, next_node_type="rnd")
