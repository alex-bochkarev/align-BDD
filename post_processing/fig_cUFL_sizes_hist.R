...
sum_sizes_cols = grep("^orig.*_AB_obj$", colnames(df_wide), value = TRUE)
all_relevant_cols = grep("^orig.*_obj$", colnames(df_wide), value = TRUE)

for (col in sum_sizes_cols) {
    df_wide[[paste(col,"rel",sep="_")]] = df_wide[[col]] / df_wide[["orig_gsifts1p_AB_obj"]]
}

for (col in setdiff(all_relevant_cols, sum_sizes_cols)) {
    df_wide[[paste(col, "rel", sep = "_")]] <- df_wide[[col]] / df_wide[["orig_gsifts1p_obj"]]
}
...
SHOW_HEU = SHOW_HEU[SHOW_HEU != "orig_gsifts1p_obj_rel"]
SHOW_HEU = SHOW_HEU[SHOW_HEU != "orig_gsifts1p_AB_obj_rel"]

df_rel = filter(df_rel, num_type %in% SHOW_HEU)

df_rel$entry_type = "obj"
